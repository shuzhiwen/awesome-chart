 ...
 */
export function tableListToObjects<K extends Meta, V = Meta>(
  tableList: RawTableList
) {
...
export function tableListToTable(tableList: RawTableList): RawTable {
  ...
  const map = new Map(
    tableList.map(([row, column, value]) => [`${row}-${column}`, value])
  )
  ...
  return [
    ...
    columns,
    rows.map((row) =>
      columns.map((column) => map.get(`${row}-${column}`) ?? NaN)
    ),
  ]
}
 ...
 */
export function relationToTable([
  nodeTableList,
  linkTableList,
]: RawRelation): RawTable {
  ...
  const nodeIds = Array.from(
    new Set(nodeTableList.slice(1).map((item) => item[idIndex]))
  )
  const nodeNames = nodeIds.map(
    (id) =>
      nodeTableList.find((item) => item[idIndex] === id)?.[nameIndex] || ''
  )
  const map = new Map(
    linkTableList.map((link) => [
  ...
  return [
    ...
    nodeNames,
    nodeIds.map((row) =>
      nodeIds.map((column) => map.get(`${row}-${column}`) ?? NaN)
    ),
  ]
}
